...
**Ключ — репозиторий и вопрос к нему, а не запрос пользователя.**
`repo:{repo_id}:{head_sha}:{prompt_version}` → `AuditResult`.
...
- Инвалидация по двум осям: смена `head_sha` (изменился код) и смена
  `prompt_version` (изменился вопрос). `AuditResult` — не свойство репозитория,
  а ответ конкретного промпта о нём, и без версии в ключе бамп до `l2-2` молча
  отдавал бы суждения `l2-1`: замер «до и после» из `CHECKLIST.md` показывал бы
  «до» оба раза, причём правдоподобными числами.
- Записи прошлых версий остаются: прошлый прогон должен воспроизводиться.
- TTL как таймер не используется. Запись живёт, пока ключ не изменился.
...
Расширение контекста слоёв или рост числа аудитов меняет эти числа заметно,
поэтому правка промптов начинается с пересчёта таблицы, а не заканчивается им.
Таблица описывает расход, а не ограничивает его.
