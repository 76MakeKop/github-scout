...
Цены за 1M токенов, проверены 2026-09-03. Сверять раз в месяц вручную;
это чтение ченджлога, а не подсистема.

| Модель | Роль | Вход off-peak / peak | Выход off-peak / peak |
|---|---|---|---|
| `deepseek-v4-flash` | интент, Слой 1 | 0,22 / 0,44 | 0,66 / 1,32 |
| `deepseek-v4-pro` | Слой 2 | 0,66 / 1,32 | 1,98 / 3,96 |
| `qwen3.8-max` | fallback Слоя 2 | 2,00 (без деления) | 6,00 |

У `deepseek-v4-pro` вход по cache hit — 0,022 / 0,044. Системные промпты слоёв
выносятся в начало сообщения без переменных частей, чтобы попадать в cache hit.
