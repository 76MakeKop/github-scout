          ...
          "items": {
            ...
            "properties": {
              ...
              "blocking": { "type": "boolean",
                            "description": "задача без этого не решается" }
            }
          }
        },
...
- `USE` — `total ≥ 0.70`, **нет блокирующих пробелов**, `code_reuse_allowed: true`.
...
**Почему у пробела есть флаг `blocking`.** Формулировка «`gaps` пуст или
тривиален» была неисполнима, пока `gaps` был списком строк: признака
тривиальности в типе не было, и код проверял только «пуст». Живой прогон дня 12
показал цену этого: `Flagsmith` с `total` 0.90 и разрешительной лицензией
проиграл `USE` одному пробелу про корпоративные функции, а `go-feature-flag`
получил `USE` лишь потому, что модель не нашла к чему придраться. Вердикт
зависел от придирчивости, а не от важности. Флаг возвращает решение коду:
модель отвечает на факт «блокирует ли этот пробел задачу», пороги и вес
лицензии по-прежнему знает только код.
